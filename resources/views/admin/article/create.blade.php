                    ...
                    <form action="{{ route('admin.article.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror" name="title"
                                id="" aria-describedby="helpId" placeholder="Enter a title" />
                            @error('title')
                        ...
                        <div class="mb-3">
                            ...
                            <select multiple class="form-select form-select-lg" name="keyword_id[]" id="">
                                ...
                                @foreach ($keywords as $rs)
                                    <option value="{{ $rs->id }}" @if (collect(old('keyword_id[]'))->contains($rs->id)) selected @endif>
                                        {{ $rs->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('desc') is-invalid @enderror" name="desc" rows="3"></textarea>
                            @error('thumbnail')
                                <span class="invalid-feedback">
                                    <strong>{{ $message }}</strong>
                                </span>
